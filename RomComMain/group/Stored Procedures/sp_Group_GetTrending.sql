CREATE PROCEDURE [group].[sp_Group_GetTrending]
    @CurrentUserId INT,
    @Page INT = 1,
    @PageSize INT = 20
AS
BEGIN
    ...
    WHERE g.[IsActive] = 1
        ...
        AND g.[CreatedDate] >= DATEADD(DAY, -30, GETUTCDATE()) -- Trending groups from last 30 days
    ORDER BY (g.[MembersCount] * 2 + g.[PostsCount] * 3) DESC, g.[CreatedDate] DESC
    OFFSET @Offset ROWS
