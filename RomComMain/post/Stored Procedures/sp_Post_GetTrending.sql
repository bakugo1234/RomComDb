CREATE PROCEDURE [post].[sp_Post_GetTrending]
    @CurrentUserId INT,
    @Page INT = 1,
    @PageSize INT = 20
AS
BEGIN
    ...
    WHERE p.[IsActive] = 1
        AND u.[IsActive] = 1
        AND p.[CreatedDate] >= DATEADD(DAY, -7, GETUTCDATE()) -- Trending posts from last 7 days
    ORDER BY (p.[LikesCount] * 2 + p.[CommentsCount] * 3 + p.[SharesCount] * 5) DESC, p.[CreatedDate] DESC
    OFFSET @Offset ROWS
